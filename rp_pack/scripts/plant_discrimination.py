...
class Plant_discriminator():
    ...
    Classifies each RGB pixel as weed or no weed (using logistic regression fitted to 3 training images)
    ...
    def runtime_routine(self):
        # fetch the latest image and apply regression
    ...
    def train_regression(self):
        ...
        # Translate the label images into binary labels
    ...
    def predict(self, img):
        ...
        (thresh, binary_predicted_image) = cv2.threshold(predicted_image, 127, 255, cv2.THRESH_BINARY) # threshold, just in case the predictions are not binary
        return cv_image, binary_predicted_image

    def component_detection(self, original, image):
        # Get connected component data of the image:
        ...
        weed_pixels = []
        # only keep the centroids of components of a certain size
        for component in range(0,output[0]):
            if self.max_component_size >= output[2][component,4] >= self.min_component_size:
                ...
                # draw the centroids on the original camera image:
